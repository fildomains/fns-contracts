...
let contracts = [
    [artifacts.require('./registry/Registry.sol'), 'Solidity']
];
...
contracts.forEach(function ([FNS, lang]) {
    contract('FNS ' + lang, function (accounts) {
        ...
        it('should allow the creation of subnodes', async () => {
            let result = await fns.setSubnodeOwner('0x0', sha3('fil'), accounts[1], {from: accounts[0]});
        ...
        it('should prohibit subnode creation by non-owners', async () => {
            await exceptions.expectFailure(fns.setSubnodeOwner('0x0', sha3('fil'), accounts[1], {from: accounts[1]}));
        });
    });
});
