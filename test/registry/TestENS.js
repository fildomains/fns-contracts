...
let contracts = [[artifacts.require('./registry/Registry.sol'), 'Solidity']]
  ...
  contract('FNS ' + lang, function (accounts) {
    ...
    it('should prohibit transfers by non-owners', async () => {
      await exceptions.expectFailure(
        fns.setOwner('0x1', '0x0000000000000000000000000000000000001234', {
          from: accounts[0],
        }),
      )
    })
    ...
    it('should prevent setting resolvers by non-owners', async () => {
      await exceptions.expectFailure(
        fns.setResolver('0x1', '0x0000000000000000000000000000000000001234', {
          from: accounts[0],
        }),
      )
    })
    ...
    it('should prevent setting the TTL by non-owners', async () => {
      await exceptions.expectFailure(
        fns.setTTL('0x1', 3600, { from: accounts[0] }),
      )
    })

    it('should allow the creation of subnodes', async () => {
      let result = await fns.setSubnodeOwner('0x0', sha3('fil'), accounts[1], {
        from: accounts[0],
      })
