...
contract('BaseRegistrar', function (accounts) {
	...
	before(async () => {
		...
		registrar = await BaseRegistrar.new(fns.address, namehash.hash('fil'), {from: ownerAccount});
		await registrar.addController(controllerAccount, {from: ownerAccount});
		await fns.setSubnodeOwner('0x0', sha3('fil'), registrar.address);
	});
	...
	it('should allow new registrations', async () => {
		...
		assert.equal(await fns.owner(namehash.hash("newname.fil")), registrantAccount);
	...
	it('should allow registrations without updating the registry', async () => {
		...
		var block = await web3.eth.getBlock(tx.receipt.blockHash);
		assert.equal(await fns.owner(namehash.hash("silentname.fil")), ZERO_ADDRESS);
	...
	it('should permit the owner to reclaim a name', async () => {
		await fns.setSubnodeOwner(ZERO_HASH, sha3("fil"), accounts[0]);
		await fns.setSubnodeOwner(namehash.hash("fil"), sha3("newname"), ZERO_ADDRESS);
		assert.equal(await fns.owner(namehash.hash("newname.fil")), ZERO_ADDRESS);
		await fns.setSubnodeOwner(ZERO_HASH, sha3("fil"), registrar.address);
	...
	it('should allow the owner to set a resolver address', async () => {
		...
		assert.equal(await fns.resolver(namehash.hash('fil')), accounts[1]);
	});
});
