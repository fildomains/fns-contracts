...
contract('RegistrarController', function () {
  ...
  it('should permit new registrations with resolver and records', async () => {
    ...
    var nodehash = namehash('newconfigname.fil')
    expect(await fns.resolver(nodehash)).to.equal(resolver.address)
    expect(await fns.owner(nodehash)).to.equal(nameWrapper.address)
    ...
    const commitment = await controller2.makeCommitment(
      ...
      [
        ...
        resolver.interface.encodeFunctionData(
          ...
          [namehash('other.fil'), 'url', 'ethereum.com'],
        ),
      ],
      false,
      0,
  ...
  it('should permit a registration with resolver but no records', async () => {
    ...
    const nodehash = namehash('newconfigname2.fil')
    expect(await fns.resolver(nodehash)).to.equal(resolver.address)
  ...
  it('should allow anyone to renew a name without changing fuse expiry', async () => {
    ...
    var nodehash = namehash('newname.fil')
  ...
  it('non wrapped names can renew', async () => {
    ...
    const nodehash = namehash(`${label}.fil`)
  ...
  it('should auto wrap the name and set the ERC721 owner to the wrapper', async () => {
    ...
    expect(await fns.owner(namehash(name))).to.equal(nameWrapper.address)
