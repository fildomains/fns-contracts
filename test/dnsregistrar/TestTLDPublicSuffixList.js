...
contract('TLDPublicSuffixList', function (accounts) {
  ...
  it('treats all TLDs as public suffixes', async function () {
    assert.equal(
      await suffixList.isPublicSuffix(utils.hexEncodeName('fil')),
      true,
    )
    assert.equal(
  ...
  it('treats non-TLDs as non-public suffixes', async function () {
    ...
    assert.equal(
      await suffixList.isPublicSuffix(utils.hexEncodeName('foo.fil')),
      false,
    )
    assert.equal(
      await suffixList.isPublicSuffix(utils.hexEncodeName('a.b.foo.fil')),
      false,
    )
  })
})
