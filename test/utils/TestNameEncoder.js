...
contract('UniversalResolver', function () {
  ...
  describe('encodeName()', () => {
    it('should encode a name', async () => {
      const result = await testNameEncoder.encodeName('foo.fil')
      expect(result['0']).to.equal(dns.hexEncodeName('foo.fil'))
      expect(result['1']).to.equal(namehash('foo.fil'))
    })
    ...
    it('should encode a long name', async () => {
      const result = await testNameEncoder.encodeName('something.else.test.fil')
      expect(result['0']).to.equal(dns.hexEncodeName('something.else.test.fil'))
      expect(result['1']).to.equal(namehash('something.else.test.fil'))
    })
  })
})
